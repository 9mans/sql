    ...
    인덱스(색인)
            ...
            * ROWID는 데이터행이 저장된 파일번호/ 블록번호/ 슬롯번호를 포함하고 있다
            ...
            인덱스 생성
                    ...
                    수동생성
                            WHERE절에 조건식으로 자주 등장하는 컬럼은 조회 성능을 향상시키기 위해 인덱스를 생성할 수 있다
    
                    인덱스 생성이 유리한 경우
                        * WHERE절에 조건식으로 자주 등장하는 경우 ENDEX를 생성하자
                        * 컬럼의 매우 다양한 값을 포함하고 있다면 인덱스를 생성하자
                        * 데이터가 매우 많은 테이블을 대상으로 데이터를 조회했을 때 조회작업을 할 때 마다 
                           전체데이터의 2% 미만으로 데이터가 조회되는 경우 해당 컬럼을 인덱스로 생성하자
                    ...
                    인덱스 생성이 필요하지 않은 경우
                       ...
                       *  대부분의 조회작업에서 전체 데이터의 2%~4% 이상 검색되는경우
