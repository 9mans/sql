...
--                                                                                                                                                     ㅣ---------------------------------------------------------ㄱ                                      
-- 신청정보 [학번, 과정번호(조인), 신청일] --- 과정정보 [과정번호(조인), 과정명, 학과번호(조인), 교수번호(조인)] --- 학과정보[학과번호(조인), 학과명] --- 교수정보[교수번호(조인), 교수명]
--                              ㄴ_______________________________ㅣ                                     ㄴ____________________________________ㅣ
...
-- 테이블 삭제하기 (삭제안됨)
DROP TABLE DEPTS;                       -- PROFESSORS, STUDENTS, COURSES에서 참조하고있다
DROP TABLE PROFESSORS;            -- STUDENTS, COURSES 에서 참조하고있다
DROP TABLE STUDENTS;                -- REQUESTS에서 참조하고있다
DROP TABLE COURSES;                  -- REQUESTS에서 참조하고있다
-- 테이블 삭제 순서 > REQUESTS(참조하는 테이블이 없음) -> COURSES -> STUDENTS -> PROFESSORS -> DEPTS

-- CACADE CONSTRAINTS는 이 테이블을 참조하는 자식 테이블에서 외래키 제약조건을 제거한 다음 테이블을 삭제한다
DROP TABLE DEPTS CASCADE CONSTRAINTS; --(제약조건을 무시하고 삭제)
