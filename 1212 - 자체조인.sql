/*
    자체 조인(조인의 대상이 같은 테이블일때)
    
    조인대상이 되는 테이블이 같은 테이블일때 발생하는 조인
    같은 테이블끼리 조인하기 때문에 서로 역할을 나누어서 별칭을 정하고 그 역할에 맞게 조인조건을 정의해야한다
    같은 테이블끼리 조인하기 때문에 조회하는 컬럼명이 동일한 경우가 있어 적절한 별칭을 붙이는 것이 좋다
*/
