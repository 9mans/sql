/*
    스키마(SCHEMA)
            ...
            * 다른 사용자가 소유하고 있는 데이터베이스 객체를 엑세스할 때는 반드시 스키마를 명시해야한다
...
/*
    시노님(SYNONYM: 동의어)
            시노님은 데이터베이스 객체(테이블, 뷰, 시퀀스 등)에 대한 대체이름(Alias)를 말한다
...
-- HR계정의 EMPLOYEES 테이블을 HTA 가 사용하는 경우
/*
    ...
    HTA 계정으로 접속해서 EMPLOYEES 조회하기 (시노님을 몰랐다면,,)
            SELECT *
            FROM HR.EMPLOYEES;
    
    HTA 계정으로 접속해서 시소님 생성하기
...
-- 시노님을 사용하면 SQL쿼리가 간결해지며 실제 데이터베이스 객체의 소유자 실제 데이터베이스 객체 이름을 숨길 수 있다
CREATE SYNONYM EMP
FOR  HR.EMPLOYEES;
