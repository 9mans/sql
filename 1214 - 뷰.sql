/*
    뷰(가상의 테이블)
        ...
        뷰는 대상으로 SELECT 작업을 주로 수행한다
        
        뷰를 사용하면 복잡한 SQL 쿼리를 단순하게 만들어준다
...
-- 부서아이디, 부서명, 관리자 아이디, 관리자 이름, 소재지 아이디, 소새지 도시, 우편번호, 주소, 국가코드, 국가명을 포함한다
...
WHERE (E.DEPARTMENT_ID, E.SALARY) IN (SELECT I.DEPARTMENT_ID, MIN(I.SALARY)
                                                                     FROM EMPLOYEES
                                                                     GROUP BY I.DEPARTMENT_ID = E.DEPARTMENT_ID);
